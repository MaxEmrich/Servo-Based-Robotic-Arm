...
void setup() {  
  ...
  my_vector = makeNewVec(1.0, 2.0, 3.5);
  my_unit_vector = makeUnitVec(my_vector);
...
void loop() {
  ...
  // Serial.println(my_vector->vectorComponents[0]);
  // Serial.println(my_vector->vectorComponents[1]);
  // Serial.println(my_vector->vectorComponents[2]);

  float newMagnitude = getMag(my_unit_vector);
  Serial.println("test");
  Serial.println(newMagnitude);
...
struct Vector* newVec_FromPoint(struct R3Point* p1, struct R3Point* p2) {    // make a new vector given two points in 3-space -- p1 is the tail, p2 is the head 
  ...
  // error checking....
  assert(
        (*newVector).tailPoint.point_3d[0] == 0.0,
